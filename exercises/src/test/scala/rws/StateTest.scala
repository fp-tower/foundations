...
class StateTest(impl: StateToImpl) extends AnyFunSuite with Discipline with Matchers {
  import impl._
  ...
  test("findCode long") {
    ...
    val instructionLines = augmentString(instructionStr).linesIterator.toList
      .traverse(_.toList.traverse(parseInstruction))
      .getOrElse(sys.error("invalid instructions"))

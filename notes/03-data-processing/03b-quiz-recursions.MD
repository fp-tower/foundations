...
The operator `+:` is used to prepend to a `List` while `:+` is used  to append to a `List`. \
For example,
...
A mnemonic is to remember that `:` in the operators `+:` and `:+` is always on the side of the `List`:
1. `data +: list`
1. `list :+ data`

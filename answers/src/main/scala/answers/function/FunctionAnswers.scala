...
object FunctionAnswers {
  ...
  def splitParFoldMap[A, B](partitions: List[List[A]])(f: A => B)(z: B, combine: (B, B) => B): B = {
    implicit val ec: ExecutionContext = fixedSize(partitions.size, "splitParFoldMap")
    Await.result(
      Future
        .traverse(partitions)(chunk => Future { foldMap(chunk)(f)(z, combine) })
        .map(foldMap(_)(identity)(z, combine)),
      scala.concurrent.duration.Duration.Inf
    )
  }

  def splitParFoldMapCommutative[A, B](partitions: List[List[A]])(f: A => B)(default: B, combine: (B, B) => B): B = {
    ...
    val ec: ExecutionContext = fixedSize(numberOfPartitions, "splitParFoldMapCommutative")
...
object FunctionAnswersApp extends App {
  ...
  time(splitParFoldMap(chunks(8, files))(wordCountByFile)(Map.empty, combineMaps))
  time(splitParFoldMapCommutative(chunks(8, files))(wordCountByFile)(Map.empty, combineMaps))
